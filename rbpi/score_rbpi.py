"""Score every RB prospect (+ UDFA supplement) with RBPI v1 ->
dashboard/rbpi_scores.json + rbpi/out/rbpi_database.csv. Mirrors score_v2.py.
Run from the repo root: `python rbpi/score_rbpi.py`."""
import warnings; warnings.filterwarnings("ignore")
import sys, os; sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import json, numpy as np, pandas as pd
import rbpi_score_v1 as W
...
pp = json.load(open(RD + "rbpi_v1_params_pre.json"))["params"]
qq = json.load(open(RD + "rbpi_v1_params_post.json"))["params"]
dm = json.load(open(RD + "rbpi_diamond.json"))
...
d["rbpi_pre"] = W.to_percentile(d["raw_pre"], ref_pre)
d["rbpi_post"] = W.to_percentile(d["raw_post"], ref_post)
d["tier"] = np.ceil(d["rbpi_post"] * 10).clip(1, 10).astype(int)
...
cal = {}
for which, col in [("post", "rbpi_post"), ("pre", "rbpi_pre")]:
    ...
    print(f"\n{which}-draft RBPI decile calibration:")
...
d["is_star_pre"] = (d["rbpi_pre"] >= STAR_PCTL).astype(int)
d["is_star_post"] = (d["rbpi_post"] >= STAR_PCTL).astype(int)
...
#   RBPI profile leads; receiving-role + carry-share are weighted up so a 3-down
...
SIM_W = {
    ...
    "rbpi_pre": 0.6, "rbpi_post": 0.6, "diamond_score": 0.6,
}
_S = d.copy()
...
COLS = ["Player", "Year", "era", "is_udfa", "udfa_type", "tier", "rbpi_post", "rbpi_pre", "raw_post", "raw_pre",
...
# fitted model stats (from fit_rbpi.py / fit_rbpi_pre2.py runs 2026-09-01)
...
out = {
    ...
    "scored": json.loads(d.sort_values(["Year", "rbpi_post"], ascending=[True, False])[COLS].round(4).to_json(orient="records")),
    "calibration": cal,
}
import shutil
os.makedirs("rbpi/out", exist_ok=True)
json.dump(out, open("rbpi/out/rbpi_scores.json", "w"), indent=1)
d[[c for c in COLS if c not in ("comp_post", "comp_pre")]].to_csv("rbpi/out/rbpi_database.csv", index=False)
shutil.copy("rbpi/out/rbpi_scores.json", "dashboard/rbpi_scores.json")
print("wrote rbpi/out/rbpi_scores.json + rbpi/out/rbpi_database.csv -> dashboard/rbpi_scores.json")
...
print("\ntop 15 (2024-2026) by RBPI post-draft:")
print(d[d.era == "prediction"].sort_values("rbpi_post", ascending=False)
      .head(15)[["Year", "Player", "tier", "rbpi_post", "rbpi_pre", "pick", "is_diamond", "is_star_post"]].to_string(index=False))
