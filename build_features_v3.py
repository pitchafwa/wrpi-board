...
ppa = pd.read_csv("data/cfbd_ppa_full.csv"); ppa["key"] = ppa.name.map(norm)
usg = pd.read_csv("data/cfbd_usage_full.csv"); usg["key"] = usg.name.map(norm)
sp = pd.read_csv("data/cfbd_sp.csv"); sp_i = sp.set_index(["year", "team"])
rec = pd.read_csv("data/cfbd_recruiting_full.csv"); rec["key"] = rec.name.map(norm)
